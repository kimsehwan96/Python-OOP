...
|프록시|퍼사드|
|---|---|
|실 객체의 대리 객체를 제공해 접근 제어|클래스의 서브시스템에 대한 인터페이스를 제공|
|타겟 객체와 동일한 인터페이스 구조를 가진다. 타겟에 대한 참조를 가진다.|서브시스템간의 의존도와 통신을 최소화 한다|
|클라이언트와 감싸고있는 객체간의 중재자 역할|퍼사드 객체는 하나의 통합된 간단한 인터페이스 제공|

여기서 프록시 패턴이, 실 객체의 대리자 역할을 함으로써 발생하는 특징들이 있다.

프록시 객체는 실 객체와 동일한 인터페이스를 가진다. 이 의미는, 동일한 상위 클래스를 상속받았다고 생각하면 되겠지?

이로서 기능을 추가해야 할 때, 실 객체의 클래스를 수정하지 않고, 프록시 객체의 클래스를 수정함으로서 기능추가가 가능하다.

이게 가능한 이유는 동일한 인터페이스를 사용하기 때문!

프록시 패턴의 단점으로는 구현을 얼마나 잘했느냐에 따라서 실 객체의 응답속도(퍼포먼스)이슈가 발생 할 수 있다는 점!
